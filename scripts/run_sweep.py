    ...
    for value in values:
        ...
        if "secondary_parameter" in sweep_config:
            ...
            for secondary_value in secondary_values:
                ...
                secondary_config["experiment"][
                    "name"
                ] = f"{config['experiment']['name']}_{parameter}_{value}_{secondary_param}_{secondary_value}"
            ...
            config["experiment"][
                "name"
            ] = f"{config['experiment']['name']}_{parameter}_{value}"
...
def run_single_experiment(
    config: Dict[str, Any], output_dir: str, seed: int = None
) -> Dict[str, Any]:
    """
...
def run_sweep(
    config_path: str, output_dir: str, seeds: List[int] = None, max_parallel: int = 1
) -> Dict[str, Any]:
    """
...
def main():
    ...
    parser.add_argument(
        "--config", type=str, required=True, help="Path to sweep configuration file"
    )
    parser.add_argument(
        "--output_dir", type=str, required=True, help="Output directory for experiments"
    )
    parser.add_argument(
        "--seeds",
        type=int,
    ...
    parser.add_argument(
        "--max_parallel", type=int, default=1, help="Maximum parallel experiments"
    )
    parser.add_argument(
        "--dry_run", action="store_true", help="Show configurations without running"
    )
    ...
    if args.dry_run:
        ...
        for i, config in enumerate(configs):
            ...
            if "model" in config:
                print(
                    f"   Model: d_model={config['model'].get('d_model', 'N/A')}, "
