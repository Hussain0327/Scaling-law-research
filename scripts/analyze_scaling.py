                ...
                if exp_dir.is_dir():
                    ...
                    if results_file.exists() and config_file.exists():
                        ...
                        row = {
                            ...
                            "checkpoint_dir": str(checkpoint_path),
                            "final_val_loss": exp_data.get("final_val_loss"),
                            "final_val_perplexity": exp_data.get(
...
def fit_scaling_law(
    x: np.ndarray, y: np.ndarray, law_type: str = "power"
) -> Dict[str, Any]:
    """
    ...
    try:
        if law_type == "power":
            ...
            r_squared = 1 - np.sum((y_clean - y_pred) ** 2) / np.sum(
...
def analyze_scaling_dimensions(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for dim, title in scaling_dimensions:
        if dim in df.columns and df[dim].nunique() > 1:
            ...
            if best_fit["success"]:
                print(
                    f"  Best fit: {best_fit['equation']} (R² = {best_fit['r_squared']:.3f})"
                )
            else:
...
def create_scaling_plots(scaling_results: Dict[str, Any], output_dir: Path) -> None:
    ...
    for dim, results in scaling_results.items():
        ...
        plt.tight_layout()
        plt.savefig(
            output_dir / f"scaling_{dim}_detailed.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
...
def create_comparison_plots(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    for i, exp_type in enumerate(experiment_types):
        ...
        plt.scatter(
            exp_data["model_parameters"],
            exp_data["final_val_loss"],
            label=exp_type,
            s=100,
            alpha=0.7,
            color=colors[i],
    ...
    if "total_steps" in df.columns:
        ...
        plt.tight_layout()
        plt.savefig(
            output_dir / "efficiency_vs_compute.png", dpi=300, bbox_inches="tight"
        )
        plt.close()

    # Plot 3: Ablation studies comparison
    ablation_experiments = [
        exp for exp in experiment_types if "ablation" in exp.lower()
    ]

    if ablation_experiments:
        fig, axes = plt.subplots(
            1, len(ablation_experiments), figsize=(6 * len(ablation_experiments), 6)
        )
        ...
        for i, exp_type in enumerate(ablation_experiments):
            ...
            if var in exp_data.columns:
                grouped = (
                    exp_data.groupby(var)["final_val_loss"]
                    .agg(["mean", "std"])
                    .reset_index()
                )
        ...
        plt.tight_layout()
        plt.savefig(
            output_dir / "ablation_comparisons.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
...
def generate_summary_table(scaling_results: Dict[str, Any], output_dir: Path) -> None:
    ...
    if summary_data:
        ...
        with open(output_dir / "scaling_laws_table.txt", "w") as f:
            ...
            f.write(
                "- Negative values indicate loss decreases as dimension increases\n"
            )
    ...
    results_summary = {
        ...
        "scaling_results": {
            dim: {
                "title": results["title"],
                "best_fit": (
                    results["best_fit"] if results["best_fit"]["success"] else None
                ),
